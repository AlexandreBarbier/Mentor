...
class DFTextField: UITextField {
    ...
    var icon: UIImage? {
        didSet {
            ...
            leftView = UIView(frame: CGRect(origin: CGPoint.zero,
                                            size: CGSize(width: img.frame.width + 16 + padding.left,
                                                         height: self.frame.height)))
            let y = (leftView!.frame.height - img.frame.height) / 2
            img.frame = CGRect(origin: CGPoint(x: 8, y: y), size: img.frame.size)
            let sepView = UIView(frame: CGRect(x: (img.center.x * 2) - 1,
                                               y: 6,
                                               width: 1,
                                               height: leftView!.frame.height - 12))
